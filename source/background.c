...
int background_functions(
  ...
  H = sqrt(rho_tot-pba->K/a/a); /* Short notations for use in the vector field*/
  pvecback[pba->index_bg_H] = sqrt(rho_tot-pba->K/a/a);

/** Pressure of the vector field as funtion of vphi, rho_r, rho_m and H.
   The function vphi_dot is the parameter \epslilon_phi (Eq. 4.13) of arXiv:1907.12216. 
   w_vf is the equations of state of the vector field (Eq. 4.18). 
   P_{vf} = w_{vf}\rho_{vf}  */
...
/* Here add P_vf_prime */
/* vphi_dot_prime is the derivative of vphi_dot in conformal time*/
...
int background_solve(
  ...
  int index_loga, index_scf, index_vf;
...
int background_initial_conditions(
 ...
 if (pba->has_vf == _TRUE_) {        
    ...
    class_test(!isfinite(pvecback_integration[pba->index_bi_phi_vf]) ||
               ...
               pba->error_message,
               "initial vphi = %e -> check initial conditions",
