...
/* Jose's test*/
  ...
  printf("Reading_Lambda %i\n",flag1);
  printf("Omega_fld = %e \n", param2);
  printf("Reading_fld %i\n",flag2);
  printf("Omega_scf = %e \n", param3);
  printf("Reading_scf %i\n",flag3);
  printf("Omega_vf = %e \n", param4);
  printf("Reading_vf %i\n",flag4);
...
/*  class_test((flag1 == _TRUE_) && (flag2 == _TRUE_) && ((flag3 == _FALSE_) || (param3 >= 0.)),
  ...
  class_test(((flag1 == _FALSE_)||(flag2 == _FALSE_)) && ((flag3 == _TRUE_) && (param3 < 0.)),
             errmsg,
             "You have entered 'Omega_scf' < 0 , so you have to unspecified 'Omega_lambda' or 'Omega_fld'.");*/
...
class_test((flag1 == _TRUE_) && (flag2 == _TRUE_) && (flag3 == _TRUE_) && (flag4 == _TRUE_),
             errmsg,
             "Only one among 'Omega_Lambda',  'Omega_fld' 'Omega_scf' and 'Omega_vf' must be left unspecified.");
  
  class_test((flag1 == _FALSE_) && (flag2 == _FALSE_),
             errmsg,
             "Only one among 'Omega_Lambda' or 'Omega_fld' must be left unspecified.");
  if (flag1 == _TRUE_) {
   class_test((flag2 == _FALSE_) && (flag3 == _FALSE_),
             errmsg,
             "Only one among 'Omega_fld' or 'Omega_scf' must be left unspecified.");  
  }
  else {
    class_test((flag1 == _FALSE_) && (flag3 == _FALSE_),
             errmsg,
             "Only one among 'Omega_Lambda' or 'Omega_scf' must be left unspecified.");  
  }
  if (flag3 == _TRUE_) {
    class_test((flag1 == _FALSE_) && (flag4 == _FALSE_),
             errmsg,
             "Only one among 'Omega_Lambda' or 'Omega_vf' must be left unspecified.");  
  }
  if (flag3 == _TRUE_) {
    class_test((flag2 == _FALSE_) && (flag4 == _FALSE_),
             errmsg,
             "Only one among 'Omega_fld' or 'Omega_vf' must be left unspecified.");  
  }
  else {
    class_test((flag3 == _FALSE_) && (flag4 == _FALSE_),
             errmsg,
             "Only one among 'Omega_scf' or 'Omega_vf' must be left unspecified.");        
  }
...
/* Jose's test*/
  printf("New values according to Friedmann constraint\n");
  ...
  /* It is necessary to read the parameters of the vector
   field even if Omega_vf is zero. Otherwise, there will have a memory 
   allocation error in the background.c module */
...
//    printf("vector field's parameters [s,b2,b3,phi0] = [%e,%e,%e,%e] \n",pba->vf_parameters[0],pba->vf_parameters[1],pba->vf_parameters[2],pba->vf_parameters[3]);
    ...
    /* Read */ 
    pba->phi_ini_vf = pba->vf_parameters[pba->vf_parameters_size-1];

  /* Jose's test*/
    printf("Phi's initial condition is %e \n",pba->vf_parameters[pba->vf_parameters_size-1]);
